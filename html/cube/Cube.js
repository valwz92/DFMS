...
function Cube(id,opts){
	...
	this.vColor=this.opts.vColor||'#111';//设定魔方材料颜色，默认是白色
...
Cube.prototype.initStyle=function(){
	...
	for(var i=0;i<this.boxsDataLength;i++){
		...
		for(var j=0;j<6;j++){
			...
			this.boxsData[i].faces[j].style.border='3px solid '+this.vColor;//设置边线 默认2px
			...
			this.boxsData[i].faces[j].style.borderRadius='10px'; //设置面的圆角，默认每小块的1/10
			//设置每个面的偏移量
...
Cube.prototype.turn3=function(type,fnComplete){
	...
	var json={y:{'coor':'y',num:0,dir:true},
			y_:{'coor':'y',num:0,dir:false},
			w:{'coor':'y',num:2,dir:true},
			w_:{'coor':'y',num:2,dir:false},
			b:{'coor':'x',num:0,dir:true},
			b_:{'coor':'x',num:0,dir:false},
			g:{'coor':'x',num:2,dir:true},
			g_:{'coor':'x',num:2,dir:false},
			r:{'coor':'z',num:2,dir:true},
			r_:{'coor':'z',num:2,dir:false},
			o:{'coor':'z',num:0,dir:true},
			o_:{'coor':'z',num:0,dir:false}
			};
